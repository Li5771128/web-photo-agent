...
- 使用模块化单体，不拆分微服务。
- 模型只负责低分辨率预览的语义视觉理解、风格解释和参数草案；图像测量、差异计算、参数安全约束、XMP 生成和清理均为确定性程序。
...
| 异步作业 | Redis 队列 | 承担分析、清理、重试与超时控制。 |
| 二进制资产 | S3 兼容对象存储 | 保存有 TTL 的上传图与预览图。XMP 按请求生成后直接下载，不持久化。 |
| 本地运行 | Docker Compose | 一次启动 Web、Worker、PostgreSQL、Redis 与对象存储；生产可替换为托管 Adapter。 |
| 模型接入 | 千问 Adapter | 使用 OpenAI 兼容 Responses API；模型标识、Base URL、提示词版本和凭证由配置注入。 |
...
| 图像分析模块 | `analyze(asset)` 返回测量结果 | RAW/EXIF 解读、直方图、色板、HSL、对比度、裁切风险、动态范围线索。原图只在本地处理。 |
| 视觉识别模块 | `recognize(reference, target)` 返回语义上下文 | 生成无 EXIF 的 1024px JPEG 预览并调用千问，识别场景、主体、人物、光线与迁移限制。 |
...
| XMP 导出模块 | `export(validatedPlan)` 返回下载响应 | XMP 序列化、允许字段白名单、白平衡排除规则、稳定标识与安全文件名。 |
...
- 图像 Worker 必须区分确定性测量和千问语义识别；识图结果可以作为用户可见的内容摘要与规划上下文，但不能冒充精确测量或已校验参数建议。
...
1. 用户首次选择 A 或 B 时，Web 应用创建匿名临时任务，并为两个角色维护独立上传槽位与上传尝试。A/B 可并行上传，旧尝试不能覆盖新选择。
2. 上传模块分别确认格式、大小和可解码性；A/B 都确认且 RAW 安全预览就绪后，用户点击“确认分析”，服务端通过原子状态转换使任务只进入一次 `queued`。普通图片上限为 30 MB；目标图 B 的受支持 RAW 上限为 40 MB。
3. Python Worker 先本地测量原始 A、B 并原子保存版本化结果，再生成低分辨率、无 EXIF 的 A/B 预览交给千问进行语义识别；RAW 由固定版本的 rawpy/LibRaw 解码。取消任务的迟到测量和模型结果都必须丢弃。两类结果分别保存后进入后续 `planning`。
4. 风格与差异模块构建规划上下文；调色计划模块产出结构化草案。
5. 参数校验模块收紧、接受或拒绝草案。成功后任务进入 `ready`，Web 仅展示此计划。
6. 用户在 `ready` 状态手动调整，或可选地请求 XMP。Web 从已校验计划同步生成文件并直接下载；导出不改变任务状态，失败也不影响参数卡。
7. 用户上传调整后的 JPG 预览图时，以同一临时任务创建新轮次，复用上一份已校验计划并重复分析流程。
8. 会话结束、任务过期或明确结束时，清理模块删除临时数据并转入 `expired`。

## 5. 数据与生命周期

PostgreSQL 只保存临时任务和结构化结果。A、B 和预览图存入对象存储并与明确的 TTL 关联；XMP 按请求生成，不持久化。
...
| XMP | 不持久化 | 用户显式请求时由已校验计划同步生成并直接下载。 |
...
- 上传校验、图像测量、模型调用、参数校验和清理记录开始时间、结束时间、状态和可安全展示的错误码；同步 XMP 请求返回明确错误码且不改变任务状态。
...
- 千问 Adapter 只接收重新编码、去除 EXIF、长边不超过 1024px 的派生 JPEG 预览以及后续脱敏测量上下文，不接收原始分辨率文件或 RAW。请求设置 `store: false`；模型、提示词与 Schema 版本显式记录，密钥只能由环境变量或受管密钥机制提供。
...
- XMP 只能从已校验计划导出，内容只含可可靠映射的全局项目且不包含曲线；导出失败时参数计划仍然可用。
