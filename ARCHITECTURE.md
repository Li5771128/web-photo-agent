...
2. 上传模块分别确认格式、大小和可解码性；A/B 都确认后通过原子状态转换使任务只进入一次 `queued`。普通图片上限为 30 MB；目标图 B 的受支持 RAW 上限为 40 MB。
3. Python Worker 先生成低分辨率、无 EXIF 的 A/B 预览交给千问进行语义识别；RAW 由固定版本的 rawpy/LibRaw 解码。取消任务的迟到模型结果必须丢弃。后续本地测量原始 A、B；两类结果分别保存后进入 `planning`。
