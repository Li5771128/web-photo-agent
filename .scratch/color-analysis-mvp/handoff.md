...
Issue 02 已完成真实千问联调和结果页验收；Issue 03 的确定性 XMP 导出已完成并通过 Lightroom Classic 初步导入验证。后续优先实现到期清理。
...
- 分析结果与 Lightroom 计划：
  ...
  - 规划失败可单独重试并复用测量与识图结果。
- 可选 XMP 导出：
  - 仅当前匿名会话中未过期、状态为 `ready` 且存在安全计划的任务可以下载。
  - Web 按固定白名单确定性生成 XMP，不调用模型、不持久化文件，也不改变任务状态。
  - 导出基础明暗、全局色彩、颜色混合和全局效果；不导出白平衡、曲线、裁切、镜头校正、蒙版或修复。
  - Lightroom Classic 初步导入验证通过；曾导致 `50000 / +150` 的白平衡字段已删除。
...
- Web 13 项回归测试、生产构建和 TypeScript 检查通过；Worker 镜像构建通过。
...
- XMP 导出失败不影响参数卡已人工验证，但尚缺 React/浏览器组件自动化覆盖。
...
- XMP 只能从已校验结构化计划确定性生成，不能直接使用模型自由文本；该边界已由生成器白名单和下载路由落实。
